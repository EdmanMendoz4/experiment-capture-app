...
class ToggleAnalyzerGUI:
    ...
    def load_data(self):
        print("Loading and pre-calculating data...")
        df_signal = pd.read_csv(self.cfg.data_csv)
        self.v_data = df_signal.iloc[:, 0].to_numpy()
        ...
        centered_signal = self.v_data - np.mean(self.v_data)
        analytic_signal = hilbert(centered_signal)
        ...
        self.amplitude_data = np.abs(analytic_signal)
    ...
    def setup_ui(self):
        ...
        # --- NEW: Indicator Lines for Macro View (Hidden by default) ---
    ...
    def update_frame(self, val):
        ...
        if self.is_macro_view:
            # --- NEW: Show and update vertical tracking lines ---
